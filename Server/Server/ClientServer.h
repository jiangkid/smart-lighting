...
public:
	...
	CString DataCheck(CHAR * buffer,int strlen,BOOL  ID,char* Infromationr,int &length);   //传输过来的数据判断
	BOOL CreateUser(CHAR * buffer);
	BOOL DeleteUser(CHAR * buffer);
	CString GetLedInformation(CHAR* buffer);
	BOOL ChangePassword(CHAR* buffer);
	BOOL InitializeClient(CHAR* buffer,CString &Area,CString &Termial,CString &Road);
...
protected:
	static int listen;
	char m_allLights[LIGHTINFOLEN*1001];   //获取灯的信息时所用到的结构体
};
